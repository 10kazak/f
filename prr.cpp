...
int** identification(int** M, int* size, bool input) {
	...
	if ((*size) < 1 || (*size) == 1) {
		printf("Ахтунг! Попробуйте другую функцию\n");
		return 0;
	}
...
int** rashiplenie(int** M, int* size) {
	...
	if (vershina > (*size) || vershina < 0) {
		printf("Ахтунг! Попробуйте другую вершину\n");
		rashiplenie(M, size);
	}
	else
	{
...
void rashiplenie_spisok(struct spisok_head** head_spisok, int* size, int** M) {
	...
	cin >> vershina;
	if (vershina > (*size) || vershina <= 0) {
		printf("Ахтунг! Нельзя расщепить такой маленький список\n");
	}
	else {
...
int main() {
	...
	h1:
	cout << " Введите количество вершин графа №1 (1-20): ";
	cin >> size1;
	if (size1 == 0 || size1 < 0 || size1 > MAX || !cin.good()) {
		cout << "Ахтунг!\n";
		cin.clear();
		goto h1;
	}
	h2:
	cout << " Введите количество вершин графа №2 (1-20): ";
	cin >> size2;
	if (size2 == 0 || size2 < 0 || size2 > MAX || !cin.good()) {
		cout << "Ахтунг!\n";
		cin.clear();
		goto h2;
	}
